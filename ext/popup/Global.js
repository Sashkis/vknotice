...
Date.prototype.toStringVkFormat = function () {
	...
	if (ago < 10)		  return window.Popup.loc('just now');
	else if (ago < 60)	  return getCase(Math.floor(ago),		window.Popup.loc('seconds')) + '&nbsp' + window.Popup.loc('ago');
		...
		if (ago < 86400) {
			...
			return window.Popup.loc('tomorrow') + '&nbsp' + window.Popup.loc('at') + '&nbsp' + h + ':' + m;
		} else {
			var y = this.getFullYear() != now.getFullYear() ? '&nbsp' + this.getFullYear() : '';
			return this.getDate() + '&nbsp' + window.Popup.loc('months')[this.getMonth()] + y + '&nbsp' + window.Popup.loc('at') + '&nbsp' + h + ':' + m;
		}
	}
};
